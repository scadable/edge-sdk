...
Local file-based storage for blobs (images, binary data, raw files).
Each partition (outbound, decoder, etc.) gets its own subdirectory.
Oldest files are automatically evicted when max_size is reached.

All operations (write, read, list, delete) are executed by the edge-main runtime.
"""
from scadable.edge import FileStorage
from scadable.edge.constants import SIZE_512_MB
...
class {class_name}(FileStorage):
    ...
    Attributes:
        id: Unique identifier for this storage.
        path: Directory path where data is stored.
        max_size: Maximum storage size in bytes. Use SIZE_* constants.
                  Oldest files are evicted when this limit is reached.
        warning_threshold: Percentage (1-100) at which the runtime logs a warning.
    """
    id = "{storage_id}"
    path = "/var/scadable/storage/{storage_id}"
...
SQLITE_STORAGE_TEMPLATE = '''\
"""
Generated by Scadable CLI.

SQLite-based storage for structured data (tables, rows, queries).
Uses WAL mode for performance. Oldest rows are evicted when max_size is reached.

All operations (insert, query, delete) are executed by the edge-main runtime.
"""
...
class {class_name}(SQLiteStorage):
    ...
    Attributes:
        ...
        max_size: Maximum database size in bytes. Use SIZE_* constants.
                  Oldest rows are evicted when this limit is reached.
        warning_threshold: Percentage (1-100) at which the runtime logs a warning.
    """
    id = "{storage_id}"
    path = "/var/scadable/storage/{storage_id}.db"
...
STORAGE_TEMPLATES = {
    "file": FILE_STORAGE_TEMPLATE,
    "sqlite": SQLITE_STORAGE_TEMPLATE,
}

# ─── Outbound Templates ──────────────────────────────────────────────────────

OUTBOUND_DATA_TEMPLATE = '''\
"""
Generated by Scadable CLI.

Scadable-managed data outbound. Streams device telemetry (Modbus registers,
OPC-UA node values, sensor readings) to the Scadable cloud via MQTT.

The system handles endpoint, certificates, and topic routing automatically.
"""
from scadable.edge import OutboundData
# Uncomment and import your device classes to filter:
# from devices.my_plc.config import MyPlc


class {class_name}(OutboundData):
    """
    Data outbound instance.

    Attributes:
        id: Unique identifier for this outbound.
        devices: List of Device classes to route here.
                 Empty list = all devices (broadcast).
    """
    id = "{outbound_id}"
    devices = []  # empty = all devices; e.g. [MyPlc] to filter
'''

OUTBOUND_UPLOAD_TEMPLATE = '''\
"""
Generated by Scadable CLI.

Scadable-managed upload outbound. Uploads files (images, binary data) from
a local storage to the Scadable cloud via S3.

The system handles bucket, presigned URLs, retry, and retention automatically.
"""
from scadable.edge import OutboundUpload
# Uncomment and import your device and storage classes:
# from devices.esp32_cam.config import Esp32Camera
# from storage.image_store.config import ImageStore


class {class_name}(OutboundUpload):
    """
    Upload outbound instance.

    Attributes:
        id: Unique identifier for this outbound.
        devices: List of Device classes to route here.
                 Empty list = all devices.
        storage: A Storage class to read files from (optional).
                 The outbound watches this storage and uploads new files.
    """
    id = "{outbound_id}"
    devices = []      # e.g. [Esp32Camera]
    storage = None    # e.g. ImageStore
'''

OUTBOUND_TEMPLATES = {
    "data": OUTBOUND_DATA_TEMPLATE,
    "upload": OUTBOUND_UPLOAD_TEMPLATE,
}
...
def cmd_add_storage(storage_type: str, storage_name: str):
    require_project()
    template = STORAGE_TEMPLATES.get(storage_type)
    if not template:
        print(f"Unknown storage type '{storage_type}'. Available: {', '.join(STORAGE_TEMPLATES.keys())}")
        sys.exit(1)

    os.makedirs("storage", exist_ok=True)
    storage_path = os.path.join("storage", storage_name)
    if os.path.exists(storage_path):
        print(f"Storage '{storage_name}' already exists.")
        sys.exit(1)

    os.makedirs(storage_path)
    config_path = os.path.join(storage_path, "config.py")
    content = template.format(
        class_name=to_class_name(storage_name),
        storage_id=storage_name,
    )
    with open(config_path, "w") as f:
        f.write(content)
    print(f"  Created {config_path}")
    print(f"\n  Next: edit {config_path} to adjust path and max_size.")

def cmd_add_outbound(outbound_type: str, outbound_name: str):
    require_project()
    template = OUTBOUND_TEMPLATES.get(outbound_type)
    if not template:
        print(f"Unknown outbound type '{outbound_type}'. Available: {', '.join(OUTBOUND_TEMPLATES.keys())}")
        sys.exit(1)

    os.makedirs("outbound", exist_ok=True)
    outbound_path = os.path.join("outbound", outbound_name)
    if os.path.exists(outbound_path):
        print(f"Outbound '{outbound_name}' already exists.")
        sys.exit(1)

    os.makedirs(outbound_path)
    config_path = os.path.join(outbound_path, "config.py")
    content = template.format(
        class_name=to_class_name(outbound_name),
        outbound_id=outbound_name,
    )
    with open(config_path, "w") as f:
        f.write(content)
    print(f"  Created {config_path}")
    print(f"\n  Next: edit {config_path} to set your device list.")
...
def main():
    ...
    p_stor = add_sub.add_parser("storage", help="Add a storage backend")
    p_stor.add_argument("type", choices=list(STORAGE_TEMPLATES.keys()), help="Storage type")
    p_stor.add_argument("name", help="Storage name (e.g. local-data, device-cache)")

    p_out = add_sub.add_parser("outbound", help="Add an outbound destination")
    p_out.add_argument("type", choices=list(OUTBOUND_TEMPLATES.keys()), help="Outbound type")
    p_out.add_argument("name", help="Outbound name (e.g. default-telemetry, image-upload)")
    ...
    if args.command == "init":
        ...
        if args.resource == "device":
            ...
            cmd_add_storage(args.type, args.name)
        elif args.resource == "outbound":
            cmd_add_outbound(args.type, args.name)
        else:
            p_add.print_help()
