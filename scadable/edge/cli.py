...
Local file-based storage. Data is stored as files on disk.
...
Operations (write, read, list, delete) are executed by the edge-main runtime.
"""
...
SQLite-based storage. Data is stored in a SQLite database with WAL mode.
Each partition (outbound, decoder, etc.) gets its own table.
Oldest entries are automatically evicted when max_size is reached.
